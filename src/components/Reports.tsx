    ...
    <div className="space-y-6">
      ...
      {reportData && (
        <>
                ...
                {reportData.topCustomers.map((customer) => (
                  <div key={customer.name} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{customer.billCount} {t('bills').toLowerCase()}</p>
                    </div>
                    <p className="font-semibold">{customer.totalAmount.toFixed(2)}</p>
                  </div>
                ))}
                {reportData.topCustomers.length === 0 && (
                  <p className="text-sm text-muted-foreground text-center py-4">{t('noCustomerDataAvailable')}</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('revenueBy')} {getReportTypeLabel(reportType)}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {reportData.revenueByPeriod.map((period) => (
                  <div key={period.period} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{period.bills} {t('bills').toLowerCase()}</p>
                    </div>
